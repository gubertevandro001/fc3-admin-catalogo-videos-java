...
public class VideoValidator extends Validator {
    ...
    protected VideoValidator(final Video video, ValidationHandler aHandler) {
        super(aHandler);
        this.video = video;
    }
    ...
    private void checkTitleConstraints() {
        ...
        if(length > TITLE_MAX_LENGTH) {
            this.validationHandler().append(new Error("'title' must be between and 1 and 255 characters"));
    ...
    private void checkDescriptionConstraints() {
        ...
        if(length > TITLE_MAX_LENGTH) {
            this.validationHandler().append(new Error("'description' must be between and 1 and 4000 characters"));
